...
def generate_numeric_debug_handle(graph_module: GraphModule) -> None:
    ...
    for node in graph_module.graph.nodes:
        unique_id = max(unique_id, node.meta.get(CUSTOM_KEY, {}).get(NUMERIC_DEBUG_HANDLE_KEY, -1))
    unique_id += 1
...
class QuantizationComparisonResult:
    ...
    def loss(self, loss_function: Callable[[torch.Tensor, torch.Tensor], torch.Tensor]) -> torch.Tensor:
...
def compare_results(
    ...
    for debug_handle, (ref_name, ref_stack, ref_stats) in ref_results.items():
        ...
        try:
            ...
            raise ValueError(f"For numeric_debug_handle={debug_handle} from ref node {ref_name} and actual node {actual_name}") from e
