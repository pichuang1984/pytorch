...
def _extract_debug_handles(model: torch.fx.GraphModule) -> Dict[str, int]:
...
class TestNumericDebugger(TestCase):
    ...
    def test_added_node_gets_unique_id(self) -> None:
        ...
        for k, v in ref_counter.items():
            self.assertEqual(v, 1, msg=f"For handle {k}, there were {v} nodes with that handle, but expected only 1")
        ...
        for name, handle in ref_handles.items():
            ...
            self.assertEqual(after_count, ref_count, msg=f"For handle {handle}, there were {after_count} nodes with that handle, but expected only {ref_count}")
